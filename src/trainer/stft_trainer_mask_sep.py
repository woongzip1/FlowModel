...
from src.flow.path_stft import ConditionalProbabilityPath, SeparateCFMPath
...
F1_DICT_TRAIN: dict[int, int] = {8: 80, 12: 128, 16: 170, 24: 256}
...
class STFTTrainerMask(Trainer):
    ...
    def _train_step(self, batch_data:dict, **kwargs):
        ...
        ## SPLIT SPECTROGRAMS   
        Y_lr = Y[...,:LOW_FREQ_BINS, :]   # [B,2,256,T]
        Z_hr = Z[...,HIGH_FREQ_START:, :] # [B,2,432,T]
        
        X0 = self.path.sample_source(Z_hr.shape, device=Z_hr.device) # [B,2,432,T]
        t = torch.rand([batch_size, 1, 1, 1], device=z.device) # [B,1,1,1]
        Xt = self.path.sample_xt(X0, Z_hr, t) # [B,2,F2,T]
        
        # self._debug_save(X0,Xt,Y,lr_preserve_mask)
        output = self.model(Xt, t, Y_lr, sr_values) # [B,2,432,T]
        target = self.path.get_target_vector_field(X0, Z_hr)
        loss = flow_matching_loss(output, target)
    ...
    def _val_step(self, batch_data:dict, idx:int, val_step_indices:list, **kwargs):
        ...
        Y_lr = Y[...,:LOW_FREQ_BINS,:]
        # Y_hr = Y[...,HIGH_FREQ_START:,:]
        Z_lr = Z[...,:LOW_FREQ_BINS,:]
        Z_hr = Z[...,HIGH_FREQ_START:,:]
        
        ## Sample t, y, and x
        t = torch.rand([batch_size, 1, 1, 1], device=z.device) # [B,1,1,1]
        x0 = self.path.sample_source(Z_hr.shape, device=Z_hr.device)
        ...
        if idx % 100 == 0:
            ...
            with torch.no_grad():
                ...
                x0_batch = self.path.sample_source(Z_hr.shape, device=Z_hr.device)
                x1_spec_batch = solver.simulate(x0_batch, ts_metric, y=Y_lr, sr_values=sr_values)
                # --- SR-adaptive concatenation
                full_specs = []
                for i in range(batch_size):
                    sr = sr_values[i].item()
                    f1_cutoff = F1_DICT.get(sr)
                    if f1_cutoff is None: raise ValueError(f"Unsupported SR for F1_DICT: {sr}")
                    
                    y_low = Y[i:i+1, ..., :f1_cutoff, :]    # [1,2,F1,T]
                    generated_hf_part = x1_spec_batch[i:i+1]
                    slice_start = f1_cutoff - HIGH_FREQ_START   # 85-80 / 170-80 / 128-80
                    if slice_start < 0: raise ValueError("f1_cutoff is smaller than HIGH_FREQ_START")
                    needed_high_part = generated_hf_part[..., slice_start:, :]
                    
                    full_spec = torch.cat([y_low, needed_high_part], dim=-2)
                    full_specs.append(full_spec)
                    
                x1_spec_batch_full = torch.cat(full_specs, dim=0)
                x1_wave_batch = self._postprocess(x1_spec_batch_full)
        ...
        if idx in val_step_indices:
            ...
            for num_steps in [5, 30]:
                ...
                # --- Run Inference ---
                x0_sample = self.path.sample_source(Z_hr[0:1].shape, device=Z_hr.device)
                ...
                # 1. Get LF part from Y
                y_low_part_sample = Y_sample[..., :f1_cutoff, :]
                slice_start = f1_cutoff - HIGH_FREQ_START
                needed_high_part_sample = x1_spec[..., slice_start:, :]
                # Concatenate
                x1_spec_full = torch.cat([y_low_part_sample, needed_high_part_sample], dim=-2)
                x1_wave = self._postprocess(x1_spec_full)
