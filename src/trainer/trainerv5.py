...
class STFTTrainer(Trainer):
    ...
    def _train_step(self, batch_data:dict, step:int, **kwargs):
        ...
        Y_lr = Y[:,:,:HIGH_FREQ_START,:] #[B,2,F1,T]
        Y_hr = Y[:,:,HIGH_FREQ_START:,:]
        Z_lr = Z[:,:,:HIGH_FREQ_START,:]
    ...
    def _val_step(self, batch_data:dict, idx:int, **kwargs):
        ...
        Y_lr = Y[:,:,:HIGH_FREQ_START,:]
        Y_hr = Y[:,:,HIGH_FREQ_START:, :]
        Z_lr = Z[:, :, :HIGH_FREQ_START, :]   # Ground-truth LR part for reconstruction
        Z_hr = Z[:, :, HIGH_FREQ_START:, :]   # Ground-truth HR part for validation loss
        ...
        if idx in [1, 5, 10]:
            with torch.no_grad():
                ...
                for num_steps in [5, 30]:
                    ...
                    # --- Run Inference ---
                    Y_hr = Y_sample[...,HIGH_FREQ_START:,:]
                    Y_lr = Y_sample[...,:HIGH_FREQ_START,:]
