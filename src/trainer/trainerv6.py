...
class STFTTrainer(Trainer):
    ...
    def _train_step(self, batch_data:dict, step:int, **kwargs):
        ...
        lr_bin_count = self.model.sr_to_lr_bins[current_sr] # 80, 160, etc.
        hf_start_bin = self.model.total_freq_bins - self.model.hr_freq_bins # 512 - 432 = 80
        ...
        # 1-step inference
        Z_hr_hat = output + (1 - self.path.sigma_min) * x0 # [B,2,F2 432,T]
        slice_start_idx = max(0, lr_bin_count - hf_start_bin)
        Z_hr_hat = Z_hr_hat[..., slice_start_idx:, :] 
        Z_hat = torch.cat((Y_lr, Z_hr_hat), dim=2)
        z_hat = self._postprocess(Z_hat)
        z_target = self._postprocess(torch.cat((Y_lr, Z_hr[...,slice_start_idx:,:]), dim=2))
    ...
    def _val_step(self, batch_data:dict, idx:int, val_idx:list, **kwargs):
        ...
        lr_bin_count = self.model.sr_to_lr_bins[current_sr]
        ...
        if idx % 100 == 0:
            ...
            with torch.no_grad():
                ...
                slice_start_idx = max(0, lr_bin_count - hf_start_bin)
                x1_spec_sliced = x1_spec_batch[:,:,slice_start_idx:,:]
                x1_spec_batch = torch.cat([Y_lr, x1_spec_sliced], dim=2)
        ...
        if idx in val_idx:
            with torch.no_grad():
                ...
                for num_steps in [5]:
                    ...
                    # --- Run Inference ---
                    Y_hr = Y_sample[...,hf_start_bin:,:]
                    Y_lr = Y_sample[...,:lr_bin_count,:]
    
                    x0_sample = self.path.sample_source(Y_hr)
                    x1_spec = solver.simulate(x0_sample, ts, y=Y_lr, sr_values=sr_values)
                    slice_start_idx = max(0, lr_bin_count - hf_start_bin)
                    x1_spec = torch.cat([Y_lr, x1_spec[...,slice_start_idx:,:]], dim=2)
                    x1_wave = self._postprocess(x1_spec)

                    min_len = min(z_sample.shape[-1], x1_wave.shape[-1])
                    z_sample_c = z_sample[...,:min_len]
                    y_sample_c = y_sample[...,:min_len]
                    
                    assert x1_wave.shape[-1] <= z_sample.shape[-1], f"Generated wave is longer! Gen: {x1_wave.shape[-1]}, GT: {z_sample.shape[-1]}"
                    assert not torch.isnan(x1_wave).any(), "NaN detected in the generated wave!"
                    assert not torch.isinf(x1_wave).any(), "Inf detected in the generated wave!"
