...
class Sampleable(ABC):
    """
    Abstract base class for 'samplable' distribution
    Distribution which can be sampled from
    """ 
    @abstractmethod
...
class DataLoaderConditionalProbabilityPathWithPrior(ConditionalProbabilityPath):
    def __init__(self, p_simple_shape: List[int], alpha: Alpha, beta: Beta, sigma_y:float = 1.0):
        p_simple = IsotropicGaussian(shape = p_simple_shape, std = 1.0)
        super().__init__(p_simple, p_data=None)
        self.alpha = alpha
        self.beta = beta
    ...
    def sample_conditional_path(self, z: torch.Tensor, t: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        """
        Samples from the conditional distribution p_t(x|z)
        Args:
            - z: conditioning variable (num_samples, c, h, w)
        ...
        # Sample xt
        # xt = t x_1 + (1-t) y + (1-t) sigma_y eps
        
        # Sample prior y
        y = y + torch.randn_like(y) * self.sigma_y
        xt = t * z + (1-t) * y
        return xt, y

    def conditional_vector_field(self, z:torch.Tensor, x0: torch.Tensor) -> torch.Tensor:
        """
        Evaluates the conditional vector field u_t(x|z)
        Args:
            - x: position variable (num_samples, c, h, w)
