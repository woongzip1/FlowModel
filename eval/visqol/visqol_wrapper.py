...
# All rights reserved.
#
# This source code is licensed under the license found in the file "LICENSE" in the same directory.
# Slightly adapted by Simon Welker, originally from: https://huggingface.co/spaces/facebook/MusicGen/blob/9cae843238aad3f5c7695a40c9ee77c42dd87aaf/audiocraft/metrics/visqol.py
# ---------------------------------------------- #
# ---------------------------------------------- #
# Adopted from 'Meta': FlowDec
# Modified version regarding audio / speech mode
# TODO: logger and debug settings 
# ---------------------------------------------- #
...
class ViSQOL:
    ...
    def __init__(self, visqol_folder: tp.Union[Path, str], mode: str = "audio",
                 debug: bool = False):
        assert visqol_folder is not None and Path(visqol_folder).exists(),\
            f"Could not find ViSQOL folder: {visqol_folder}"
        self.visqol_folder = str(visqol_folder)
        self.visqol_mode = mode
        self.target_sr = self._get_target_sr(self.visqol_mode)
        
        # detect mode 
        if self.visqol_mode == 'audio':
            self.model = "libsvm_nu_svr_model.txt"
            self.use_lattice_model = False
        elif self.visqol_mode == "speech":
            self.model = 'lattice_tcditugenmeetpackhref_ls2_nl60_lr12_bs2048_learn.005_ep2400_train1_7_raw.tflite'
            self.use_lattice_model = True
        
        self.debug = debug
        assert Path(self.visqol_bin).exists(), \
            f"Could not find the ViSQOL binary at: {self.visqol_bin}"
    ...
    def visqol_model(self):
        return os.path.join(self.visqol_folder, 'model', self.model)

    @property
    def visqol_bin(self):
        return os.path.join(self.visqol_folder, 'bazel-bin', 'visqol')
    ...
    def _run_visqol(
        ...
        cmd = [
            ...
            '--use_lattice_model', ('true' if self.use_lattice_model else 'false'),
        ]
        ...
        if result.returncode or not os.path.isfile(results_csv_path):
    ...
    def __call__(
        ...
        try:
            if os.path.isdir(tmp_dir) and input_csv and results_csv:
                self._run_visqol(
                    input_csv,
                    results_csv,
            ...
            logger.exception("Exception occurred when running ViSQOL: %s")
            return np.nan
        finally:
