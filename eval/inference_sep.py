...
def run_ode_inference(solver, trainer, lr_audio, num_timesteps=50, sr_value=8, device='cuda'):
    ...
    x0_sample = trainer.path.sample_source(Z_hr.shape, Z_hr.device).to(device)
    ...
    y_low = Y[0:0+1, ..., :f1_cutoff, :]    # [1,2,F1,T]
    generated_hf_part = x1_spec[0:0+1]
    ...
    generated_audio = trainer._postprocess(full_spec)
